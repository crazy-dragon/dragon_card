...
数据数组（对象数组）。**每条必须带 `item_order`（从 1 递增的整数）**，它是导入对齐与覆盖更新的关键：

- 导入时按 `item_order` 对齐：同名序号覆盖更新，新数据未包含的旧序号会被移除（保留学习进度）
- **重复导入不会产生重复卡片**，可用新文件整体替换旧数据
- 无 `item_order` 的数据会按追加处理，重复导入会产生重复，且无法覆盖更新
...
> 数据字段可平铺（如 `{"item_order": 1, "word": "..."}`）或包在 `data` 里（如上例），导入均支持。
